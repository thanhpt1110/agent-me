...
5. **Run the bridge**:
   ...
   Auto SFA collects five required fields: `display_name` (DevTest `Automation Dev Linux` display name, for example `Thanh Phan`), `destination_folder_id` (the DevTest week release folder id, such as `05-2026/Week3-4`), `url_path`, `start`, and `end`. The dashboard pre-fills `source_folder_id` as `50722`; keep it unless the run needs a different pool/source folder. Optional `task_ids` switches the run to `uv run dtoperator.py sfa -i "<ids>" --user-login "<display_name>" -f`; otherwise it releases tasks matching the display name. The runner overwrites source and destination folder IDs at run time. The dashboard asks for per-run DevTest `USERNAME`/`PASSWORD` by default so `magic-auto` logs in as that user and extracts that user's `loginid`; checking "use default" falls back to host credentials. The password is not written to server config or job history. Slack uses the default credentials already loaded on the host and points credential overrides to the dashboard. Dashboard and Slack Auto SFA triggers are also recorded in `auto_sfa_runs` for the dashboard's scrollable trigger history.
