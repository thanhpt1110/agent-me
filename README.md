...
5. **Run the bridge**:
   ...
   Auto SFA collects five required fields: `display_name` (DevTest `Automation Dev Linux` display name, for example `Thanh Phan`), `destination_folder_id` (the DevTest week release folder id, such as `05-2026/Week3-4`), `url_path`, `start`, and `end`. Optional `task_ids` switches the run to `uv run dtoperator.py sfa -i "<ids>" --user-login "<display_name>" -f`; otherwise it releases tasks matching the display name. The dashboard can send per-run DevTest `USERNAME`/`PASSWORD` overrides to the child process without writing them to server config or job history; Slack uses the default credentials already loaded on the host and points credential overrides to the dashboard.
