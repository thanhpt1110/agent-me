...
5. **Run the bridge**:
   ...
   Auto SFA collects five required fields: `username_email` (for example `thaphan@nvidia.com`, mapped to `--user-login thaphan`), `destination_folder_id` (the DevTest week release folder id, such as `05-2026/Week3-4`), `url_path` (GitLab Merge Request / MR link), `start`, and `end`. It expands those shared values into `/localhome/local-thaphan/magic-auto/configs.json`, then runs `uv run dtoperator.py sfa --user-login "<login>" -f` from the `magic-auto` repo while posting new terminal log lines back into the same thread.
