...
bash "$REPO_ROOT/scripts/setup-codex-mcps.sh"
...
The remaining steps need a human at a browser:
  ...
  a) Log in to Codex (one-time per machine):
        codex login

  b) Refresh the MaaS token store used by Codex MCPs (NVIDIA SSO):
        uv run agent-me-codex-reauth
     The helper refreshes the bearer-token source used by Codex MCPs and
     opens/prints auth URLs. Sign in to NVIDIA SSO; tokens persist according
     to MaaS policy.

  c) Verify everything is healthy:
        codex mcp list                        # should show registered MaaS MCPs
